...
function CardOffer ({offer }:CardOfferProps ):JSX.Element {
  const {previewImage, isPremium, price, type, title, id, rating, isFavorite} = offer;
  const dispatch = useAppDispatch();
  const selectedOffer = useAppSelector((state) => state.HOTELS.selectedOffer);
  const onListItemHover = () => dispatch(changeOffer(id));

  return (
    <article
      onMouseOver={() => {onListItemHover();}}
    ...
    >
      ...
      <div className="place-card__info">
        <div className="place-card__price-wrapper">
          ...
          < FavoriteButton isFavorite={isFavorite} className={'place-card'} width={18} height={19}
            id={selectedOffer}
          />
        </div>
